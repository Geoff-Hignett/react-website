			...
			<div className="h-96 bg-home-hero bg-contain relative">
				<div className="max-w-theme h-full mx-auto flex items-center justify-between px-3">
					<div className="w-full lg:w-1/3 font-poppins text-lg text-white">
						Lorem ipsum dolor sit amet consectetur adipisicing elit. Ducimus earum laudantium doloremque aliquam atque nisi?
					</div>
							...
							{latestThreePosts.map(({ id, date, title }) => (
								<Link key={title} href={`/blog/${id}`} className="border-b-2 border-dotted cursor-pointer hero-post mb-12">
									<p className="font-courgette text-white mb-3">
										By John Smith <span className="text-black font-poppins font-bold">{`- ${date}`}</span>
									</p>
				...
				<div className="flex flex-col md:flex-row justify-between ">
					{latestThreePosts.map(({ id, imagePath, category, date, extract, title }) => (
						<div key={id} className="w-full md:w-[30%] mb-10">
							<div className="relative sm:h-[23vw] h-[275px]">
								<Image className="absolute bottom-0 left-0 object-cover" src={imagePath} fill alt="" />
